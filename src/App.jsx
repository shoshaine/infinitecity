...
const useMockMarketCap = () => {
  ...
  const [target, setTarget] = useState(16200)
  
  useEffect(() => {
    const interval = setInterval(() => {
      setMcap(prev => {
        const diff = target - prev
        // Smooth movement with some noise
        const step = diff * 0.005 + (Math.random() - 0.5) * 50
        const next = prev + step
        
        // Occasionally change target to create "up and down" movement
        if (Math.random() > 0.99) {
          setTarget(Math.random() > 0.7 ? 16200 : 3450 + Math.random() * 5000)
        }
        
        return Math.max(3000, Math.min(25000, next))
      })
    }, 50)
        ...
        <div className="bg-white/5 backdrop-blur-xl p-6 rounded-2xl border border-white/10 shadow-2xl">
          ...
          <div className="flex items-baseline gap-2">
            <p className="text-5xl font-mono font-bold text-white">
              ${mcap.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
            </p>
          ...
          <div className="mt-4 h-1 w-full bg-white/10 rounded-full overflow-hidden">
            <div 
              ...
              style={{ width: `${Math.min(100, (mcap / 16200) * 100)}%` }}
            />
          </div>
        </div>
